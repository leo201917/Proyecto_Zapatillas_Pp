      ...
      <div class="content-wrapper">
       ...
       <form name="frmAdd" method="post" action="" id="frmAdd"
         ...
         <div id="mail-status">
           ...
           <div>
           <label style="padding-top: 20px;">Empresa a Facturar</label> <span
            id="name-info" class="info"></span><br /> 

            <select class="form-control" width = "20" onchange="getval(this);" name = "codigoEmpresaFacturar" id = "codigoEmpresaFacturar">
            <option value=""><?php echo $result[0]["codigoEmpresaFacturar"]; ?></option>
            <?php
            require_once "../model/EmpresaFacturar.php";
            $db_handle_c = new DBController();
            $empresaFacturar_e = new EmpresaFacturar();
            $resultCombo = $empresaFacturar_e->getallEmpresaFacturar();
            foreach ($resultCombo as $k => $v):
            echo '<option value="'. $resultCombo[$k]["codigoEmpresaFacturar"].'">'. $resultCombo  [$k]["razonSocial"].'</option>';
            endforeach;
            ?>
            </select><br>
            <label style="padding-top: 20px;">Cliente</label> <span
            id="name-info" class="info"></span><br /> 

            <select class="form-control" width = "20" onchange="getval(this);" name = "codigoCliente" id = "codigoCliente">
            <option value=""><?php echo $result[0]["codigoCliente"]; ?></option>
            <?php
            require_once "../model/Cliente.php";
            $db_handle_c = new DBController();
            $cliente_c = new Cliente();
            $resultCombo = $cliente_c->getallCliente();
            foreach ($resultCombo as $k => $v):
            echo '<option value="'. $resultCombo[$k]["codigoCliente"].'">'. $resultCombo  [$k]["nombres"] , ' ' ,$resultCombo  [$k]["apellidos"].'</option>';
            endforeach;
            ?>
            </select><br>
        <label style="padding-top: 20px;">Fecha de Emision</label> <span
            id="name-info" class="info"></span><br /> <input type="datetime-local"
            name="fechaEmision" id="fechaEmision" class="demoInputBox"
            value="<?php echo $result[0]["fechaEmision"]; ?>"><br>
        <label style="padding-top: 20px;">Estado Comprobante</label> <span
            id="name-info" class="info"></span><br /> <input type="text"
            name="estadoComprobante" id="estadoComprobante" class="demoInputBox"
            value="<?php echo $result[0]["estadoComprobante"]; ?>"><br>
        <label style="padding-top: 20px;">Tipo de Moneda</label> <span
            id="name-info" class="info"></span><br /> <input type="text"
            name="tipoMoneda" id="tipoMoneda" class="demoInputBox"
            value="<?php echo $result[0]["tipoMoneda"]; ?>"><br>
    </div> 
    <div>
...
function validate() {
    ...
    if(!$("#codigoEmpresaFacturar").val()) {
        $("#name-info").html("(required)");
        $("#codigoEmpresaFacturar").css('background-color','#FFFFDF');
        valid = false;
    }

    if(!$("#codigoCliente").val()) {
        $("#name-info").html("(required)");
        $("#codigoCliente").css('background-color','#FFFFDF');
        valid = false;
    }

    if(!$("#fechaEmision").val()) {
        $("#name-info").html("(required)");
        $("#fechaEmision").css('background-color','#FFFFDF');
        valid = false;
    }

    if(!$("#estadoComprobante").val()) {
        $("#name-info").html("(required)");
        $("#estadoComprobante").css('background-color','#FFFFDF');
        valid = false;
    }

    if(!$("#tipoMoneda").val()) {
        $("#name-info").html("(required)");
        $("#tipoMoneda").css('background-color','#FFFFDF');
        valid = false;
    }

    if(!$("#monto").val()) {
        $("#name-info").html("(required)");
        $("#monto").css('background-color','#FFFFDF');
        valid = false;
    }
