    ...
    <form name="frmAdd" method="post" action="" id="frmAdd"
         ...
         <div id="mail-status">
           ...
           <div>
            ...
            value="<?php echo $result[0]["nombres"]; ?>"><br>

            <label style="padding-top: 20px;">Apellidos</label> <span
            id="name-info" class="info"></span><br /> <input type="text"
            name="apellidos" id="apellidos" class="demoInputBox"
            value="<?php echo $result[0]["apellidos"]; ?>"><br>
            
            <label style="padding-top: 20px;">Correo</label> <span
            id="name-info" class="info"></span><br /> <input type="text"
            name="correo" id="correo" class="demoInputBox"
            value="<?php echo $result[0]["correo"]; ?>"><br>

            <label style="padding-top: 20px;">Password</label> <span
            id="name-info" class="info"></span><br /> <input type="password"
            name="password" id="password" class="demoInputBox"
            value="<?php echo $result[0]["password"]; ?>"><br>

            <label style="padding-top: 20px;">Sexo</label> <span
            id="name-info" class="info"></span><br /> <input type="text"
            name="sexo" id="sexo" class="demoInputBox"
            value="<?php echo $result[0]["sexo"]; ?>"><br>

            <label style="padding-top: 20px;">Fecha De Nacimiento</label> <span
            id="name-info" class="info"></span><br /> <input type="text"
            name="fechaNacimiento" id="fechaNacimiento" class="demoInputBox"
            value="<?php echo $result[0]["fechaNacimiento"]; ?>"><br>

            <label style="padding-top: 20px;">DNI</label> <span
            id="name-info" class="info"></span><br /> <input type="text"
            name="dni" id="dni" class="demoInputBox"
            value="<?php echo $result[0]["dni"]; ?>"><br>

            <label style="padding-top: 20px;">Sueldo</label> <span
            id="name-info" class="info"></span><br /> <input type="text"
            name="sueldo" id="sueldo" class="demoInputBox"
            value="<?php echo $result[0]["sueldo"]; ?>"><br>

            <label style="padding-top: 20px;">Estado</label> <span
            id="name-info" class="info"></span><br /> <input type="text"
            name="estado" id="estado" class="demoInputBox"
            value="<?php echo $result[0]["estado"]; ?>"><br>

            <label style="padding-top: 20px;">Cargo</label> <span
            id="name-info" class="info"></span><br /> 

            <select class="form-control" width = "20" onchange="getval(this);" name = "codigoCargo" id = "codigoCargo">
            <option value=""><?php echo $result[0]["codigoCargo"]; ?></option>
            <?php
            require_once "../model/Cargo.php";
            $db_handle_c = new DBController();
            $cargo_c = new Cargo();
            $resultCombo = $cargo_c->getallCargo();
            foreach ($resultCombo as $k => $v):
            echo '<option value="'. $resultCombo[$k]["codigoCargo"].'">'. $resultCombo  [$k]["descripcion"].'</option>';
            endforeach;
            ?>
            </select>   
    </div> 
            ...
            function getval(sel)
{
    ...
    <script>
      document.querySelector('#codigoCargo').value = <?php echo $result[0]["codigoCargo"]; ?>
    </script>
    <script>
function validate() {
    var valid = true;   
    ...
    if(!$("#correo").val()) {
        $("#name-info").html("(required)");
        $("#correo").css('background-color','#FFFFDF');
        valid = false;
    }
    if(!$("#password").val()) {
        $("#name-info").html("(required)");
    ...
    if(!$("#sueldo").val()) {
        $("#name-info").html("(required)");
        $("#sueldo").css('background-color','#FFFFDF');
        valid = false;
    }
    ...
    if(!$("#codigoCargo").val()) {
        $("#name-info").html("(required)");
        $("#codigoCargo").css('background-color','#FFFFDF');
        valid = false;
    }
    return valid;
}
