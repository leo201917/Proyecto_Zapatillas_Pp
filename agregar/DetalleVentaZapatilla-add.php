...
?>
    ...
    onSubmit="return validate();">
      ...
      <div>
        <label style="padding-top: 20px;">CODIGO VENTA</label> <span
            id="name-info" class="info"></span><br /> <input 
            name="codigoVentaZapatilla" id="codigoVentaZapatilla" class="demoInputBox"value =" <?php 
            require_once "../model/VentaZapatilla.php";
            $db_handle_ = new DBController();
            $ventazapatilla = new VentaZapatilla();
            $resultUltimo = $ventazapatilla->getUltimaVenta();
            foreach ($resultUltimo as $k_ => $v_):          
             echo $resultUltimo[$k_]["codigoVentaZapatilla"]; 
             $VariableVentaCabv = $resultUltimo[$k_]["codigoVentaZapatilla"];   
           endforeach;
            ?>"> 
      </div>  
      ...
      <div>
        <label style="padding-top: 20px;">SUBTOTAL</label> <span
            id="name-info" class="info"></span><br /> <input 
            name="subTotalVenta" id="subTotalVenta" class="demoInputBox" type="text">
      </div>
...
//    if (key==13){
  ...
  function onclickFunction (){
    var cantidad = $('#cantidad').val(); 
     ...
     document.getElementById('subTotalVenta').value= subtotal;
...
function validate() {
    ...
    if(!$("#subTotalVenta").val()) {
        $("#name-info").html("(required)");
        $("#subTotalVenta").css('background-color','#FFFFDF');
        valid = false;
    }
    ...
    if(!$("#codigoVentaZapatilla").val()) {
        $("#name-info").html("(required)");
        $("#codigoVentaZapatilla").css('background-color','#FFFFDF');
        valid = false;
    }
